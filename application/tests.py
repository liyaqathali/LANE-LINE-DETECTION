...
def getVideoPath(VidImg):
    TEST_IMAGES_Video = os.getcwd() + '//application//test_images//' + VidImg
    print(TEST_IMAGES_Video)
    return main(TEST_IMAGES_Video)


# if __name__ == '__main__':
ret, mtx, dist, rvecs, tvecs = calibrate_camera(calib_images_dir= os.getcwd() +'//application//camera_cal')
def main(TEST_IMAGES_Video):
   #ret, mtx, dist, rvecs, tvecs = calibrate_camera(calib_images_dir= os.getcwd() +'//application//camera_cal')
    clip = VideoFileClip(TEST_IMAGES_Video).fl_image(process_pipeline)
    clip.write_videofile(os.getcwd() +"//media//output.mp4", audio=False)
